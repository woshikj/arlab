...
function SpawnShape(gotoCam)
{
	...
	mesh.velocity = {x:Math.random() * 2 - 1,y:Math.random() * 4 + 0.5,z:Math.random() * 0.9 + 0.1};
		...
		if(!textAppeared)
		{
			...
			if(scanText)
			{
				scanTextTimeout = setTimeout(function()
				{
					...
					scanText.innerHTML = '<span style="position:absolute;left:-17%;top:350%;display:flex;justify-content: center;">Tap to continue</span>';
				},2000);
				scanText.innerHTML = '';
			}
			addedScanText = false;
...
function update()
{
	...
	textGroup.scale.x = markerRoot1.scale.x * 0.75;
	textGroup.scale.y = markerRoot1.scale.y * 0.75;
	textGroup.scale.z = markerRoot1.scale.z * 0.75;
